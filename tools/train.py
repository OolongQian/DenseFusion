...
parser.add_argument('--dataset_root', type=str,
                    default='/home/sucheng/Robotics/perception/DenseFusion/datasets/ycb/YCB_Video_Dataset',
	...
	for epoch in range(opt.start_epoch, opt.nepoch):
		...
		for rep in range(opt.repeat_epoch):
			for i, data in enumerate(dataloader, 0):
				...
				points, choose, img, target, model_points, idx = Variable(points).cuda(), Variable(
					choose).cuda(), Variable(img).cuda(), Variable(target).cuda(), Variable(
					model_points).cuda(), Variable(idx).cuda()
		...
		for j, data in enumerate(testdataloader, 0):
			points, choose, img, target, model_points, idx = data
			points, choose, img, target, model_points, idx = Variable(points).cuda(), Variable(choose).cuda(), Variable(
				img).cuda(), Variable(target).cuda(), Variable(model_points).cuda(), Variable(idx).cuda()
			pred_r, pred_t, pred_c, emb = estimator(img, points, choose, idx)
